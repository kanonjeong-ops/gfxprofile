...
class Plugin:
    ...
    def apply_all(self, profile, confirm_token=None):
        ...
        try:
            ...
            #   바뀌었을 수 있다**(바로 위와 같은 이유: 엔진은 config를 먼저 쓴다).
            #   **전 게임의 outcome을 그대로** 남겨야 "무엇이 바뀌었을 수 있는가"를 로그만으로
            #   좁힐 수 있다.
            #   ⚠️ 예전에는 그 예로 *"`.applied` 기록 실패"*를 들었는데 **그 쓰기는 13판에서
            #     삭제됐다**(설계 §14-A · A11). 지금 `.applied`를 만드는 코드는 **0줄**이고
            #     `store.APPLIED_NAME`은 **옛 버전이 남긴 파일을 부산물로 걸러 내는 데만** 쓴다.
            #     결론(전 게임 outcome을 남긴다)은 그대로 참이라 예만 걷어 냈다.
    ...
    def add_game(self, appid, config_path, name=None, confirm_token=None):
        ...
        ★★ `config_path`를 **여기서 검증하지 않는다**(설계 §3-A). 이 값은 진짜 절대경로여야
          ...
          `check_path`(G11, 심볼릭 링크·prefix 밖) → `assert_config_candidate`(G14, `.sav` 등
          **5갈래** — 열거의 정본은 그 함수의 독스트링이다).
