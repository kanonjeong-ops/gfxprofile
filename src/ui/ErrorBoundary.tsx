 ...
 *   그래서 영어 리터럴로 고정한다.
 *   ⚠️ **이 예외의 범위를 잠그던 검사(`test_i18n_sets.py`)는 삭제됐다**(2026-08-15 UI 검사 계열
 *     정리 — 사용자 지정). 예외가 넓어지지 않게 하는 것은 이제 **이 파일의 import 목록뿐**이다:
 *     맨 위 줄이 `react` 하나이고 i18n을 들이지 않는다. 그 줄이 유지되는 한 `t()`를 부를 방법이
 *     없다 — 검사가 아니라 **의존 없음**이 방어선이므로, 여기에 import를 더하지 말 것.
 *
 * ★ 폴백을 **검사 대상 컴포넌트로 그리지 않는다**(`PanelSection` 등). `index.tsx`의
 *   self-check 실패 안내와 같은 원칙이다 — 순수 intrinsic 요소만 쓴다.
 *
 * ★ 화면 문구에 **내부 식별자를 쓰지 않는다** — 그것은 옛 표시명이다. 진단 태그(`where`)는
 *   로그에만 둔다.
 *   ⚠️ **화면에 남은 옛 이름을 막던 검사(`test_frontend_display_name.py`)는 삭제됐다**
 *     (2026-08-15 UI 검사 계열 정리 — 사용자 지정). 지금 이 규칙을 강제하는 것은 없다.
 *
