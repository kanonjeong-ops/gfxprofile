 ...
 * 팝업 D — 게임 감지.
 *
 * 타이핑이 하나도 없다: Game Mode에는 온스크린 키보드가 자동으로 뜨지 않아 appid나 경로를
 *   손으로 치게 만들면 이 화면은 핸드헬드에서 못 쓴다. 입력 요소를 하나도 두지 않는다 —
 *   목록에서 고르거나 파일 선택기로 고른다.
 *
 * 500게임 규모 대응 장치는 둘이다: ① 「등록된 게임 숨기기」(기본 ON) ② 확신 후보 일괄 등록.
 *   잔여(애매한 것)는 사람이 봐야 하는 항목이라 원리상 더 줄일 수단이 없다.
 */

const COLUMN_STYLE = { display: "flex", flexDirection: "column", gap: "10px" } as const;
/* 잔글씨 3종은 전부 한국어 문장을 그린다 — `wordBreak: "keep-all"`로 낱말 중간 줄바꿈을
   막는다(사유·제외 규칙의 정본은 `popup.tsx`의 `KEEP_ALL_STYLE` 주석).
   이 파일에서 경로를 그리는 자리는 `CANDIDATE_PATH_STYLE` 하나뿐이고 거기는 뺐다. */
...
/** [파일 직접 고르기] 아래 설명 줄 — 버튼 가로 패딩과 같은 들여쓰기로 축을 공유한다. */
const DESC_STYLE = {
...
/** [파일 직접 고르기] — 설명 줄을 아래 거느리는 세로 스택 버튼이다. */
...
/** 행 버튼 2종은 폭이 고정이다 — 행마다 x좌표가 같아야 수직 이동 중 커서가 안 튄다. */
...
/**
 * 후보 행의 버튼 — [선택]과 [이 파일로 추가]가 같은 자리·같은 폭이다.
...
/**
 * 수동으로 고른 파일에서 appid를 읽는다 — 타이핑 0회를 지키면서 자동 스캔이 놓친 Proton
 * 게임을 등록하는 방법이다.
 *
 * 네이티브 리눅스 게임은 이 경로로도 등록되지 않는다. appid를 prefix 경로에서 읽으므로
 *   prefix 밖 파일은 어느 게임의 것인지 알 방법이 없고, 설령 알아도 `check_path`(G11)가
 *   prefix 밖을 거부한다.
 *
 * 이것은 가드가 아니라 식별자 추출이다 — 잘못 읽어도 경계는 그대로다(문은 엔진 하나).
 */
...
/**
 * 경로를 뒤에서부터 줄인다 — prefix 경로는 앞이 길고 앞쪽은 전부 같다.
...
/** 배지 — 상태를 말로 그린다. 주황 테두리는 없다 — 미등록은 정상 상태다. */
 ...
 * 후보 카드 하나.
 *
 * 미등록 행: 확신이면 `[추가]`(1탭) · 애매하면 `[후보 보기/접기]`, 그리고 어느 쪽이든 행별
 *   `[파일 직접 고르기]`. 행별 선택기는 탐지 후보가 전부 틀렸을 때의 게임별 탈출구라 없애지 않는다.
 * 등록된 게임에는 버튼을 두지 않는다 — `add_game`이 이미 등록된 appid를 거부하고, 화면에서
 *   그 경로를 아예 만들지 않는 것이 가드보다 확실하다.
 */
function EntryCard({
  entry,
  busy,
  ...
  entry: DiscoverEntry;
  busy: boolean;
  /** 이 행에 착지시킬 것인가 — 팝업 D의 초기 포커스는 첫 후보 카드다. */
  focused: boolean;
  ...
  return (
    /* 두 축이 같은 카드를 가리킨다(팝업 G의 `GameCard`와 같은 관용구 — 그 주석이 정본):
         `preferredFocus`는 진입 컨테이너(`renderMain`의 `preferredChildEntryProps`)를 지나
         포커스가 들어올 때 읽히고, `autoFocus`는 마운트 시점에 스스로 요청한다.
    ...
    >
      <div style={CARD_INNER_STYLE}>
          ...
          {entry.registered ? null : (
            ...
            <Focusable {...ROW_FLOW} style={{ display: "flex", gap: "4px", flex: "0 0 auto" }}>
              {entry.confident && entry.best ? (
                /* 확신 단일 매치는 모달 없이 1탭이다: 후보가 하나뿐이라 펼쳐 봐야 고를 것이
                   없고, 경고도 구조적으로 발생하지 않는다. */
                <PopupButton
                  disabled={busy}
        ...
        {/* 애매한 게임의 후보를 화면 안에서 펼친다(모달이 아니다).
            선택 즉시 등록 전환: 후보 버튼이 `[선택]`에서 같은 자리 `[이 파일로 추가]`로 바뀌고
              두 번째 A로 등록된다. 포커스 이동만으로는 등록되지 않는다 — 등록에는 두 번의
              의도적 누름이 필요하다. */}
...
export function DiscoverPopup({
  ...
  /**
   * 변이를 QAM에 알린다 — 확정 실행이면 성공·실패를 가리지 않는다(엔진은 쓴 뒤에 거부할 수
   * 있다). 화면 갱신은 이 팝업이 스스로 한다 — 다른 일이다.
   */
  onMutate?: () => void;
  ...
  /** 기본값은 숨김이다 — 이 화면에 오는 이유는 「아직 등록 안 한 것」을 찾기 위해서다. */
   ...
   *
   * 진입 시점엔 아직 「훑는 중」이라 고를 카드가 없고 카드는 비동기로 도착한다. 그래서 진입
   *   컨테이너의 `PREFERRED_CHILD`만으로는 첫 착지가 늦다.
  ...
  /**
   * 등록 — 후보 1탭 등록·후보 선택·파일 선택기가 같은 함수를 쓴다.
   *
   * 2단계 계약이다. 경고가 나오는 파일은 첫 호출에서 저장되지 않고 `CONFIRM_REQUIRED`가 온다 —
   *   그때 확인창을 띄우고, 확정하면 받은 토큰을 그대로 되돌려 재호출한다. 취소하면 registry는
   *   1바이트도 안 바뀐 상태다.
   * 어느 경로에서 확인창이 뜨는지는 백엔드가 정한다. 프론트가 `manual` 여부로 고르면 판정이
   *   두 곳에 생기고, 언젠가 어긋나면 무방비가 되는 쪽은 프론트다.
   * 성공 note는 남은 백업 수로 갈린다: 등록을 해제해도 백업은 남으므로, 다시 등록한 순간
   *   「복원할 것이 있다」를 그 자리에서 말해야 한다. 0이면 그 줄이 없다.
   */
  const register = useCallback(
    (appid: string, path: string, name?: string, token?: string) =>
      runMutation(() => addGame(appid, path, name, token), "DISCOVER_ADD_FAILED", (res) => {
        if (!res.ok) {
          // CONFIRM_REQUIRED는 오류 표시 대상이 아니라 확인 흐름 신호다.
          //   공통 문이 토큰 발급 응답으로 식별해 재조회·변경 통지를 생략한다.
        ...
        // 이름은 백엔드가 확정한 값이다. 수동 등록에는 프론트가 아는 이름이 없다.
  ...
  /**
   * 하단 상주 선택기 — 조건 없이 항상 그린다.
   *
   * 이 버튼을 0건 분기 안에 넣으면 안 된다. 탐지도 등록도 10인 기기에서는 「전부 이미 등록돼
   *   있습니다」만 뜨고 진입점이 사라지는데, 그것이 자동 스캔이 놓친 Proton 게임을 등록할
   *   유일한 길이다. 조건이 없으면 어긋날 조건도 없다.
   * 시작 위치는 봉투가 준 라이브러리 경로뿐이다. 근거가 없을 때 `"/"` 같은 값을 지어내지
   *   않는다 — 파일 시스템 루트는 이 도메인의 시작점이 아니다(게임 prefix는 열 단계 아래다).
   *   모르면 그 자리에서 다시 읽는다. 그 응답이 곧 시작점이다.
   */
  const openManual = useCallback(
    (root: string) =>
      pickConfigFile(`${root}/steamapps/compatdata`).then((path) => {
        if (!path) return;                       // 선택 결과 없음 — 조용히 끝난다.
  ...
  const pickManual = useCallback(
    () => {
      ...
      // 아직·또는 다시 모르는 상태다. 조회는 읽기 전용이라 재조회·통지가 붙지 않는다.
      return runQuery(load, "LOAD_FAILED", (res) => {
        ...
        // 라이브러리를 못 찾은 것은 모르는 상태다 — 아무 데서나 열지 않고 그 사실을 말한다.
        if (!root) {
  ...
  /** 확신 후보 일괄 등록 — 대상 수는 봉투가 준 값이고 프론트가 다시 세지 않는다. */
  const runBulk = useCallback(
    () =>
      runMutation(() => registerConfident(), "DISCOVER_ADD_FAILED", (res) => {
        ...
        // 없었던 것을 사건처럼 말한다(`RESET_OK`+`RESET_LEFT_NOTE`와 같은 문법).
  ...
  /**
   * 감지 제외 해제 — 한 버튼 한 쓰기다. 재포함은 등록이 아니다: 이 호출 뒤 그 게임은 감지
   * 목록에 후보로 다시 뜰 뿐이고, 등록은 사용자가 거기서 한다.
   * 그래서 `DISCOVER_INCLUDED_NOTE`가 다음 경로를 병기한다 — 안 그러면 「포함했는데 왜
   * 그대로냐」가 된다.
   */
  const runInclude = useCallback(
    // 제외 목록과 후보 목록이 함께 바뀐다 — 봉투를 통째로 다시 읽는다(훅이 붙인다).
    (row: ExcludedRow) =>
  ...
  /** 숨긴 것이 있으면 숨겼다고 말한다(「감지가 안 되네」라는 오독을 차단한다). */
  const hiddenN = hideRegistered && counts ? counts.registered : 0;

  /**
   * 요약 줄의 글자 — 한 상태에 한 문장이다.
   *
   * 「훑는 중」과 「로딩 중」을 두 자리에서 각각 그리면 같은 상태를 두 번 말하고, 조회가
   *   실패했을 때는 그 중 하나가 영영 「훑는 중」으로 남아 화면이 거짓을 말한다. 그래서 상태를
   *   여기 한 곳에서 고른다: 값이 있으면 요약 · 없고 사유가 있으면 침묵(note가 말한다) ·
   *   둘 다 없으면 훑는 중.
   */
  ...
  function renderTail() {
    ...
    /* 0건 안내는 「게임이 없다」고 말하지 않는다. 탐지는 `compatdata/<appid>/pfx` 구조에 묶여
       있어 네이티브 리눅스 게임은 스캔 대상 자체가 아니다 — 그 한계를 화면이 말하지 않으면
       사용자는 「설치한 게임이 없다」로 읽는다. */
    return (
  ...
  function renderMain() {
    return (
      /* 기본 뷰의 진입 컨테이너다(팝업 G `renderList`와 같은 자리·같은 근거):
         후보 카드의 `preferredFocus`는 이 선언이 있는 컨테이너를 지나 포커스가 들어올 때만
         읽힌다(정본은 `preferredChildEntryProps` 주석). 선언을 빼면 문서 순서상 첫 focusable인
         [다시 검색]이 포커스를 가져간다.
         자리는 여기 하나뿐이다 — 액션 줄·필터·목록을 함께 품어야 하므로 스크롤 목록에 붙이면
           팝업 진입이 그 바깥에서 시작해 다시 [다시 검색]에 선다. */
      <Focusable {...preferredChildEntryProps} style={COLUMN_STYLE}>
        ...
        {/* [다시 검색]이 먼저다: 진입 직후의 A 관성이 착지하는 자리가 무해한 쪽(다시 읽기)
            이어야 한다. 일괄 등록은 대상이 0이면 아예 그리지 않는다 — 눌러도 아무 일이 없는
            버튼은 라벨이 거짓말을 한다. */}
        ...
        {/* 제외 진입은 목록 위 전용 행이다: 하단에 두면 도달 비용이 목록 길이에 비례해
            「존재를 보인다」는 논거가 게임패드에서 성립하지 않는다.
            0건이면 비활성으로 남긴다 — 복구 렌즈의 유일한 화면 발견 경로라 숨기지 않는다. */}
        <Focusable>
          <PopupButton
        ...
        {/* 이 버튼이 무엇을 하는 것인지 그 자리에서 말한다(자기 설명). */}
  ...
  function renderExcluded() {
    return (
      <div style={COLUMN_STYLE}>
        ...
        {/* 「등록 해제」 어휘와 맞춘 통일 문구다 — 다시 포함하면 무엇이 되는지를 조건까지
            말한다. */}
        ...
        <PopupScrollList>
          {excluded.map((row) => (
            <Focusable key={row.appid} {...listRowNavProps} style={CARD_STYLE}>
              {/* 안쪽은 후보 카드와 같은 이중 구조다(위 `EntryCard`와 같은 관용구):
  ...
  return (
    <GfxPopup title={t("DISCOVER_TITLE")} icon={<IconSearch />} where="popup-d" closeModal={closeModal}>
      {/* 본문은 반드시 `renderBody`를 지난다: 폴백 모드에서 오버레이와 원 콘텐츠가 동시에
          떠 있는 상태를 호출부가 만들 수 없게 하는 장치다. */}
