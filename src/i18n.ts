...
// ★ 2026-08-06 실기에서 드러난 것: 언어 확정이 **한 화면 컴포넌트 안에서만** 일어나면, 그
//   화면을 거치지 않고 다른 화면이 먼저 마운트될 때 `current`가 기본값 "en" 그대로다 —
//   한국어 사용자에게 **영어 화면이 뜬다**(실기로 확인). 그래서 "언어를 확정한다"를
//   **화면과 무관한 모듈 수준의 일**로 옮긴다. 지금 호출 지점은 QAM 한 곳이지만(P15),
 ...
 *  ⚠️ **중복 방지는 모듈 인스턴스 안에서만 성립한다** — QA R3 실측: 실기에서 화면이 둘일 때
 *  (당시 QAM 패널 + 전체 화면) 인스턴스를 공유하지 않아 각각 1회씩 `ui_hello`가 나갔다(같은
 *  백엔드 세션 토큰·7~8초 간격으로 재로드가 아님이 확인됨). 패널 재오픈마다 나던 3번째 호출은
 *  사라졌다. 호출 지점이 QAM 하나가 된 지금(P15)은 1회다. 더 줄이는 것은 Decky 런타임 문제다. */
/**
