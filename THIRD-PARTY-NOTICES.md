...
- The complete source of the bundled version is included in this repository
  at `third_party/decky-api-1.1.3/`: the published npm package without its
  prebuilt `dist/` (step 1 below rebuilds it from this source), plus the
  `tsconfig.json` from the upstream `v1.1.3` tag. The version is pinned in
  `pnpm-lock.yaml`.
