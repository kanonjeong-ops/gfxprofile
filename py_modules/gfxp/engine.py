    ...
    **하나가 실패해도 나머지를 계속 진행한다.** 모드를 바꿔 부팅한 상황에서 "5개 중 2개만
    ...
    for appid in sorted(reg["games"]):
        ...
        try:
            ...
            #   막는다.** 디스크가 이미 어느 슬롯과 같으면 이 경로는 **0개**를 쌓는다(아래
            #   `state["matches"]` 갈래). `src/limits.ts`가 같은 사실을 *"게임을 안 띄우고
            #   적용만 반복 → 0개/회 → 대피본이 밀리지 않는다"*로 적어 두었고, 이 주석이
            #   한동안 그것과 정반대를 말했다(QA R2 ADVISORY-R2-02).
