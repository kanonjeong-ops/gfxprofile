...
def _trusted_entries(appid, plan):
    ...
    ★ 꼬리 길이는 `len(plan) + 1`이다 — 아직 안 쓴 계획 건수에 **이 쓰기 한 건**(지금 판정 중인
      계획 밖 쓰기)을 더한 값이 이 동작이 앞으로 밀어낼 칸 수다. `plan`은 쓸 때마다 줄므로
      호출마다 다시 재도 맞는다.
    ⚠️ **안 잰 잔여 — `+1`은 계획 밖 쓰기를 「한 건」으로 본다.** 한 동작에서 계획 밖 쓰기가
      **둘 이상 겹치면** 꼬리를 그만큼 **과소평가**한다. 틀리는 방향은 **안전하지 않은 쪽**이다 —
      실제로는 축출될 사본을 「살아남는다」고 보아 근거로 삼고 **무쓰기로 접을 여지**가 생긴다.
      도달 조건은 D-01보다 좁다(한 동작에서 **본체 여럿이 동시에** 계획↔쓰기 사이에 바뀌어야 한다).
      **재지 않았다** — 도달 불가라는 뜻이 아니라 측정하지 않았다는 뜻이다.
      정확히 하려면 `plan`이 **원래 크기와 소비 이력**을 같이 들고 다녀야 하는데 그건 **API 변경**이라
      이번 범위에서 하지 않았다(발주자 확인 2026-08-23).
    ★ `plan`이 없으면(단일 쓰기) 축출도 자기 것 하나뿐이라 링 전체가 근거다 — 종전 그대로다.
    """
    entries = list_backups(appid)
    if plan is None:
        return entries
    return entries[:max(0, BACKUP_KEEP - len(plan) - 1)]
