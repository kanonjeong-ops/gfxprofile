...
def slot_holds(appid, profile, sha1, filename=None):
    ...
    `filename`을 주면 **그 이름으로 남아 있는지**까지 본다 — 이름이 달라지면 슬롯에 남는 파일이
    실제로 바뀌므로 *"달라지는 것이 없다"*가 거짓이 된다.
    ★ **4인자 소비자는 수를 세지 말고 이름을 적는다**(규칙 4). 4인자로 부르는 곳:
      · `engine.save_profile` — 크기 가드의 기준값을 믿을지 판정(`trusted`)
      · `engine.save_profile` — 저장의 무쓰기 판정
      · `confirm.needs_confirm` — "이미 같다" 조기반환(확인창을 띄울지 판정, QA R1 D-1로 합류)
      · `confirm._slot_materials` — 확인창 표시재료(크기·해시)를 믿을지 판정(QA DEFECT-04)
      ⚠️ 첫째는 **2026-08-23 D8이 새로 만든 소비자**인데 그때 이 문장이 *"둘"*이라 적힌 채였다.
        소비자를 늘리면 **이 목록에 이름을 더한다** — 개수로 적으면 늘린 사람이 안 고친다.
    """
...
def plan_backups(entries, items):
    ...
    **판정은 동작이 시작할 때 여기서 한 번**이고, 아래 `make_backup`은 그 결과를 집행한다.
    ...
    ★ 축출 자리를 자르는 식은 **위 `_doomed_tail` 하나**이고 `restore.ring_observe`도 그것을 부른다.
      식을 여기 옮겨 적지 않는다 — 예전에 이 줄이 `entries[BACKUP_KEEP - adding:]`이라 인용해
      두었는데 **실물은 그 뒤 `max(0, …)`가 붙었고**(N-02) 인용만 옛 모양으로 남았다.
...
def _trusted_entries(appid, plan):
    ...
    ★★ **축출될 사본은 근거가 아니다**(D-01, 2026-08-23). 이 함수가 없던 판본은 계획 밖 키를
      **「호출 시점의 링」**으로 판정했는데, 그것이 바로 D1이 계획 경로에서 고친 병이다 —
      먼저 쓴 대피본이 근거 사본을 밀어내므로, 호출 시점에 보이는 사본이 **그 동작이 끝나면
      없을 수 있다.** 실측: 링 10칸 포화 + 최고령이 「바뀐 뒤 내용」의 유일한 사본일 때
    ...
    ⚠️ **자기 쓰기를 꼬리에 세면 자기실현 회로가 된다**(N-01, 2026-08-23 — 실측으로 재현했다).
      *"이 쓰기가 일어난다"*를 가정하면 그 가정이 꼬리를 키우고 → 증인이 꼬리에 들어가고 →
      ...
      정상 중복이 무쓰기로 접히지 않고 쓰여, **확인창이 예고하지 않은 백업이 삭제됐다.**
      (`plan_backups`는 같은 순환을 **고정점**으로 푼다. 여기는 고정점이 필요 없다 —
       *"안 썼을 때"*를 물으면 순환 자체가 생기지 않는다.)
    ⚠️ **`BACKUP_KEEP - len(plan)`으로 세면 안 된다** — 그 식은 링이 **가득 찼다**고 가정한다.
      비포화 링에서는 밀려나는 칸이 그보다 적고, 그만큼 살아남을 증인을 **불신하게 된다**(N-01).
    ★ `plan`은 쓸 때마다 줄고 `entries`는 매번 다시 나열하므로, 호출마다 다시 재도 맞는다.
    ★ `plan`이 없으면(단일 쓰기) 축출도 자기 것 하나뿐이라 링 전체가 근거다 — 종전 그대로다.
    """
...
def make_backup(appid, data, tag, filename, plan=None):
    ...
    ★★ **같은 태그에 같은 내용이 이미 있으면 아무것도 쓰지 않는다**(설계 §14-G ⓔ). 디스크
      ...
      정상 동작이다. **호출부 전부가 여기를 지난다** — 쓰기의 문은 그대로 이 함수 하나다.
      호출부는 `remove._evacuate` · `engine.save_profile` · `engine.apply_profile` ·
      `engine.restore_backup`(**두 자리**)이다. **호출부를 늘리면 여기 이름을 더한다** —
      개수로 적으면 늘린 사람이 안 고치고, 「다섯」은 줄을 센 것인지 함수를 센 것인지도
      말하지 않는다(규칙 4 · QA R2 ADVISORY-R2-07이 지목한 자리다).
    ...
    ★ **계획은 「이것만 써라」가 아니라 「이건 중복이라도 써라」는 허가 목록이다**(사용자 결정
      ...
      ⚠️ **그 자체 판정의 근거는 「호출 시점의 링」이 아니다**(D-01, 2026-08-23 — 실측으로 전손을
        재현했다). 위 `_trusted_entries`가 **이번 동작이 축출할 꼬리를 빼고** 넘긴다. 호출 시점의
        링을 그대로 보면 **D1이 계획 경로에서 고친 병이 이 경로에 그대로 되살아난다** — 먼저 쓴
        대피본이 밀어낼 사본을 근거로 삼아 무쓰기가 되고, 그 사본이 잘려 나가 내용이 사라진다.
        **「자체 판정」을 `backup_holds(list_backups(appid), …)`로 되돌리지 마라.**
