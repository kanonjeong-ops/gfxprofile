...
- `engine.py` → **당시엔 diff fence 위반이었다.** 그때 엔진은 M1 실사용 코드와 기록된 diff 밖으로
  움직이지 않았다. ⚠️ **그 fence는 15판에서 폐지됐다**(설계 §14-D, 사용자 결정) — 엔진·store 수정은
  이제 다른 코드와 같은 규율을 받는다. **이 절은 「왜 이 파일이 생겼나」의 이력이지 현재 구속이 아니다.**
- `main.py`  → *"접착층은 정책 판단 금지"* 위반 (**이쪽은 지금도 유효하다**)
...
"이미 적용됨"을 보고했다. 소비자마다 판정 기준이 달라 "중간 상태는 전부 합법"이 붕괴한다.
→ **적용 계열 소비자의 공통 판정 기준인 meta.json을 먼저 지운다.** 그러면 어느
지점에서 멈춰도 `_profile_ready`와 `apply_all`의 already가 **함께** "없음"이라 판정이 갈리지
않고, "프로필 없는 등록 게임"과 동형이라 **다시 삭제하면 완결**된다(신규 복구 코드 0줄).

⚠️ **「meta가 없으면 어느 소비자도 프로필로 판정하지 않는다」는 D5(2026-08-22) 이후 거짓이다.**
삭제 확인창(`delete_preview`)은 기록이 아니라 **본체**로 판정하므로, meta unlink 뒤 rmtree 전의
중간 상태를 **"프로필 있음"**으로 본다. 그리고 그것이 **옳다** — 본체가 남아 있으면 되찾을 내용이
있고, 다시 삭제하면 그 본체가 정상적으로 대피된다. 위 순서의 목적은 *"모든 소비자를 끈다"*가
아니라 **"적용 계열의 두 판정이 서로 어긋나지 않게 한다"**이고, 그 목적은 그대로 유효하다.

⚠️ **위 「already는 meta만 본다」는 지금 시제로 읽으면 거짓이다** — 그 서술은 **초판 순서를 왜
바꿨는가**의 이력이다. `engine.apply_all`의 already 판정은 그 뒤 **본체 실측**(`store.slot_holds`)으로
바뀌었고(사용자 결정 「적용의 「이미 적용됨」 판정을 본체 실측으로」), 그래서 *"본체 없음 · meta 잔존"*
에서는 `_profile_ready`와 already가 **둘 다 「없음」**이라 그 갈림 자체가 이미 닫혀 있다.
→ 순서를 그대로 두는 근거는 이제 *"판정이 갈린다"*가 아니라 **"어디서 멈춰도 중간 상태가 한 종류"**다.
(행번호를 달지 않는다 — 이 인용은 세 판 연속으로 밀렸다. 이름으로만 가리킨다.)
...
def _paths_in_position(appid):
    ...
    ⚠️ `backups/<appid>`까지 보는 이유(Codex #1): 1단계 대피가 `store.make_backup`→
      ...
      너머 디렉터리를 열거해 초과분을 `os.unlink` 한다 — 루트 밖 파일 삭제다.
      **위험은 지금도 실재한다**(2026-08-23 실측: `prune_backups`를 직접 부르면 링크 너머
      루트 밖 파일이 실제로 지워진다). 그래서 **이 문에서** 막는다.
      ⚠️ **예전 근거였던 *"store는 fence 대상이라 못 고친다"*는 이제 성립하지 않는다** —
        그 fence는 15판에서 폐지됐다(설계 §14-D). **그래도 자리를 옮기지 않는 이유는 둘이다**:
        ⓐ `store.py`는 **정책 판단을 하지 않는 층**이다(그 파일 머리 규약 — 정책은 위층에 둔다).
        ⓑ 경로 정책을 `prune_backups`에도 적으면 **같은 규칙이 두 곳에 살고**, 이 함수가 이미
          세 갈래(삭제 문·지문·미리보기)의 **공용 술어 하나**라는 구조가 깨진다.
