#!/usr/bin/env python3
"""**단일 게임 route도 한 번만 관측한다** — 설계 §14-G ⓕ.

### 무엇이 틀렸었나
`apply_profile`·`save_profile`·`delete_game`·`restore_backup`은 **고지를 만들 때 한 번, 지문을
낼 때 또 한 번** 파일을 읽었다(삭제는 세 번, 백업 나열만 네 번). 그 두 관측 사이에 상태가
바뀌면 화면은 **옛 상태**를 이름으로 말하고 토큰은 **새 상태**에 묶인다 — 그 토큰은 2차 호출에서
그대로 통과하고, 사용자는 *자기가 본 적 없는 상태*를 승인한 것이 된다.
`apply_all`·`reset_all`은 처음부터 *"축출 예고와 그 지문은 한 번의 산출에서 같이 나온다"*였고
같은 변조를 걸면 거부됐다 — **세 결함이 아니라 한 구조**이고, 이 파일은 그 비대칭이 사라졌는지를
잰다.

### 어떻게 재는가 — 창은 주입해야만 열린다
실측 창이 183~217µs라 자연 발생을 기다릴 수 없다. 그래서 `store.list_backups`를 감싸
**첫 호출이 값을 돌려준 직후 링을 한 칸 민다**(아주 오래된 stamp의 백업 파일 하나를 넣는다 —
포화 링에서 **축출 대상이 바뀌는** 자리다).
    · 관측이 둘이면 → 고지는 옛 링, 지문은 **새 링**. 토큰이 새 상태에 묶여 2차 호출에서 **통과**한다.
    · 관측이 하나면 → 고지도 지문도 옛 링. 2차 호출에서 링이 달라 토큰이 **거부**된다.
...
#: 주입할 백업 — stamp가 아주 오래라 **목록의 맨 끝**(=가장 먼저 잘릴 자리)에 들어간다.
#: 포화 링에서 개수만 보는 신호는 이것을 못 잡고, 이름 순서를 재는 지문만 잡는다.
    ...
    try:
        ...
        def patched(appid):
            ...
            if target is not None and str(appid) == str(target):
                state["armed"] = None
                # ★ 고지와 지문 **사이**에 상태를 밀어 넣는 자리다. 파일을 직접 쓴다 —
        ...
        try:
            for i, route in enumerate(ROUTES):
                ...
                # ★ 정면 계측: 확인 단계의 백업 나열이 **몇 번인가**. 둘 이상이면 그 사이에
                #   낄 창이 있다는 뜻이고, 아래 거부는 우연히 성립한 것일 수 있다.
                if observed != 1:
