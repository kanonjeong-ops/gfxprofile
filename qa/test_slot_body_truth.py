#!/usr/bin/env python3
"""**슬롯이 실제로 무엇을 들고 있는가** — 기록(meta)이 아니라 본체로 판정한다 (R14 #4·#5).

두 판정이 슬롯 `meta.json`의 기록만 믿고 있었다:
  · 적용의 *"이 내용은 다른 슬롯에 보존돼 있다"* → 그러면 **묻지 않고 진행 + 대피 생략**이다.
    기록이 거짓이면(본체가 없거나 다른 내용) 게임 설정 파일의 **마지막 온전한 사본이 사라진다.**
  · 슬롯 복원의 *"이미 같습니다"*(`already`) → 그러면 **아무것도 안 한다.**
...
그 사이에 전원이 나가면 **본체는 새 내용, meta는 옛 기록**이 된다 — 아래 픽스처가 그 상태다.
본체만 사라지는 쪽은 `rmtree` 중단 잔재로 이미 알려져 있다(`test_delete_game` ②의 픽스처).

이 파일이 잠그는 것:
  ⓐ 다른 슬롯 **본체가 없으면** 적용은 묻고, 대피본을 만든다(그 내용이 백업에 남는다)
  ⓑ 다른 슬롯 **본체가 다르면**(meta만 일치) 역시 묻고 대피한다
  ⓒ 음성 대조군 — 슬롯이 멀쩡하면 예전처럼 **묻지 않고 대피도 안 한다**(과잉 대피 방지)
  ⓓ 슬롯 복원의 `already`가 **본문**으로 판정된다 — 손상된 슬롯을 복원이 고친다
  ⓔ 목록의 `same_as_target` 배지도 같은 판정을 쓴다(화면과 동작이 같은 근거)
...
def main_test():                                                # noqa: C901  (시나리오 나열)
    ...
    try:
        ...
        def half_write(appid, profile, data):
            ...
            쓰기 도중 전원이 나가면 실제로 이렇게 남는다(store.write_profile의 두 atomic_write)."""
        ...
        cfg2.write_bytes(B)                             # 디스크 = internal **meta**와 같은 내용
        main._CONFIRM_TOKENS.clear()
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ⓓⓔⓕ 슬롯 복원 — `already`와 배지가 **본문**으로 판정된다
        # ═══════════════════════════════════════════════════════════════════
