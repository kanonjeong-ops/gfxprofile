#!/usr/bin/env python3
"""복원은 그 행의 「되돌릴 곳」으로 간다.

행 종류와 무관하게 게임 설정 파일만 되돌리면, 프로필을 덮어쓰기 직전에 대피시킨 사본을
되돌려도 정작 그 프로필은 그대로다 — 실기에서 보고된 결함이다. `profile_*` 행은 그 슬롯으로 간다.

잠그는 것:
  ⓐ `profile_*` 복원이 슬롯을 백업 내용으로 바꾼다 (meta·본체 둘 다)
  ⓑ 그때 게임 설정 파일은 안 바뀐다  ← 목적지를 뒤바꾼 구현에서 ⓐ와 동시에 뒤집힌다
  ⓒ 슬롯의 옛 내용이 대피본으로 1건 남는다
  ⓓ 슬롯 == 백업이면 `already`이고, 백업 개수는 변하지 않는다
  ⓔ 실행 중이어도 거부되지 않는다(프로필 슬롯은 게임이 읽지도 쓰지도 않는다)
  ⓕ 비-dict meta ∧ 본체 있음 → 확인 요구 + `slot_unreadable`; 확정 뒤 슬롯 복구,
     백업 개수 불변
  ⓖ meta 있음 ∧ 본체 없음 → meta가 같아도 `already`가 아니며 슬롯 복구,
     백업 개수 불변
  ⓗ `disk` 행은 반대 — 게임 설정 파일만 바뀌고 슬롯은 그대로다
  ⓘ 정상 슬롯에서는 `evacuates=True`·비어 있지 않은 `saved_at`을, 본체 없는 슬롯에서는
     `evacuates=False`·빈 `saved_at`을 검사한다. 비-dict meta에서는 `evacuates=False`만 검사한다.

ⓕ는 meta 부재 갈래를 실행하지 않는다.

### ⓘ가 메우는 사각 — 「대피 0건」은 재고 있었는데 화면이 대피를 약속하는지는 안 쟀다
ⓕ·ⓖ는 링이 안 움직이는 것까지만 봤다. 그런데 확인창은 같은 봉투를 읽고 "복원 전에 지금의
내용도 백업으로 보관합니다 — 백업 한 칸을 씁니다"와 "덮어쓸 대상(현재 …): {saved_at}에
저장됨"을 그린다. 그 갈래에서는 둘 다 거짓이다(대피할 파일이 없고 그 자리에 내용도 없다).
봉투가 그 둘을 그대로 실어 보내면 사용자는 없는 프로필을 덮어쓴다고 믿고, 쓰지도 않을 백업
한 칸을 지불한다고 믿는다. 잠그는 것은 봉투의 두 값이고, 화면이 그 값으로 줄을 그리는지는
실기 몫이다(UI는 여기서 재지 않는다).

음성 대조군은 ⓑ·ⓗ 자체다: 목적지 판정을 지우거나 뒤바꾼 구현에서 이 둘이 반드시 깨진다.
ⓘ의 음성 대조군은 아래 ⓐ 구간이다: 같은 두 값이 거기서는 참·비어 있지 않음이라야 한다.
한쪽만 재면 "항상 False로 실어 보내는" 구현이 통과한다.
...
def main_test():                                                # noqa: C901  (갈래 나열)
    ...
    try:
        ...
        def mkgame(appid, name):
            """슬롯 둘 + 프로필 대피본 둘. `disk` 대피본은 만들지 않는다(마지막 적용 줄)."""
        ...
        if not is_confirm(ask):
            ...
            return finish(problems, tmp)
        # ── ⓘ 음성 대조군: 이 구간에서는 두 사실이 참이다(대피가 실제로 일어나고 그 자리에
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ⓕ meta가 손상됐고 본체는 남아 있다 (대피 불가를 고지하고 진행)
        # ═══════════════════════════════════════════════════════════════════
        ...
        if row10 is None or meta10.get("sha1") != store.sha1_bytes(INTERNAL):
            ...
            # ── ⓘ 이 갈래: 위에서 링이 안 움직인 것을 쟀다. 화면이 그 사실과 같은 말을 하는가.
            if is_confirm(ask):
                if params_of(ask).get("evacuates"):
