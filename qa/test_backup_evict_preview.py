#!/usr/bin/env python3
"""축출 고지가 실제로 지워지는 파일과 같은가.

확인창은 지워질 백업의 이름을 대며 승인을 받는다(`BACKUP_EVICT_NOTE`). 그 목록이 실제
`store.prune_backups`가 지우는 것과 다르면, 화면이 지워지지 않을 파일 이름을 대고 정작
지워지는 것은 말하지 않는다 — 고지가 오정보가 되는 자리다.

### 이 파일의 핵심은 정렬 기준이다
순서가 두 벌이면 갈린다. prune이 자르는 목록과 화면이 그리는 목록이 다른 키를 쓰면, 화면이
댄 이름과 실제로 사라지는 파일이 어긋난다. 지금 순서의 정본은 `store.backup_order_key`
하나이고, 이 파일은 그것을 두 축으로 잰다 — ① 예고 == 실제 삭제 ② 사전순으로 고른 답은 이
픽스처에서 다른 파일을 가리킨다(음성 대조군).

잠그는 것:
  ⓐ 적용 확인창의 `evicted`가 실제 지워진 파일과 일치한다
  ⓑ 복원 확인창의 `evicted`도 같은 산출 함수로 같은 답을 낸다
  ⓒ 링이 안 찼으면 `evicted`는 빈 배열이다(없는 손실을 고지하지 않는다)
  ⓓ 무쓰기(already) 갈래에는 고지가 실리지 않는다
  ⓔ prune 순서 == 화면 순서이고, 둘 다 사전순이 내는 답과 다르다
  ⓕ 같은 초·같은 파일명 두 백업이 표기에서 갈린다 — 아래.

### ⓕ가 메우는 사각
ⓐ~ⓔ의 픽스처는 파일명을 갈라 두어 같은 표기가 겹치는 세계를 만들지 않는다. 그런데 그 세계는
정상 API로 쉽게 생긴다 — 같은 초에 같은 설정 파일이 두 번 대피하면 `…-disk-video.ini` /
`…-disk-1-video.ini`가 되고, 화면이 그리는 조각 `(stamp_label, filename)`이 글자 하나까지 같다.
확인창은 그 이름으로 승인을 받으므로 사용자는 무엇을 승인했는지 모른 채 승인한다. ⓕ는 그
세계를 실제로 만들어 잰다.

### 못 재는 것
  · 확인창이 그 목록을 실제로 그리는지는 UI라 여기서 재지 않는다(실기 몫). ⓕ가 잠그는 것은
    봉투가 구별 가능한 값을 싣는가까지다.
...
def main_test():                                                # noqa: C901  (시나리오 나열)
    ...
    try:
        ...
        if lex_last == prune_order_last:
            ...
            return finish(problems, tmp)
        # 순서 키가 꼬리로 놓는 것은 seq 0인 COLLIDE_A다 — 사전순은 COLLIDE_B를 가리킨다.
        ...
        #   삭제를 약속하는 것이고, 「백업 한 칸을 씁니다」도 거짓말이 된다.
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ⓑ 복원 확인창도 같은 함수로 같은 답을 낸다
        # ═══════════════════════════════════════════════════════════════════
        prune_next = os.path.basename(store.list_backups(APPID)[-1])
        expected_next = [as_item(prune_next)]
        # 행을 고르기 전에 게임 설정 파일 상태를 확정한다 — `same_as_target`은 그 시점의
        #   사실이라, 고른 뒤에 파일을 바꾸면 고를 때 본 근거가 무효가 된다.
        # 링에 아직 없는 내용을 쓴다: 대피본이 실제로 만들어져야 축출이 일어나고, 그래야
        ...
        try:
            ...
            #   두 번째는 아예 안 만들어진다. 이 절이 재려는 것은 표기의 모호성
        ...
        if len(set(shown)) != len(shown):
            P("★ⓕ-2 **화면 표기가 두 항목에서 같다** — 사용자는 무엇을 승인하는지 알 수 없다 (%s)"
              % shown)
        # 번호의 뜻: 같은 표기를 공유하는 것 중 순서 키가 앞세우는 것이 1번이다.
