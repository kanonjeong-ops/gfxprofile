#!/usr/bin/env python3
"""일괄 적용은 하나가 실패해도 나머지를 계속한다.

엔진은 게임마다 예외를 격리하고 각 결과 행을 돌려준다. 이 파일은 `engine.apply_all`을
직접 호출해 실패한 게임과 계속 적용된 게임, 전체 행 수를 잰다. route·확인 토큰·화면 버튼은
이 파일의 단언 범위가 아니다.

합성 데이터만 쓴다 — 실사용 게임 설정 파일을 건드리지 않는다. 실데이터로 돌리면 그 게임의
현재 설정이 실제로 덮어써진다.
"""
import os
import pathlib
...
def main():
    ...
    try:
        ...
        #    파일을 지우는 것은 실패가 아니다 — `atomic_write`가 새로 만들어 복구해 준다.
        #    실제로 쓰기가 막히게 만든다: 폴더에서 쓰기 권한을 뺀다.
        ...
        # `code or note`는 너무 느슨하다. 화면은 `code`로 "조치가 같은 것"과 "게임마다 다른 것"을
        #   가른다 — `note`(한국어 자유문)로는 못 가른다. 그러니 `code`가 반드시 있어야 하고,
        #   그것이 없으면 화면이 오정보를 낸다.
