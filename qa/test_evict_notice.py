#!/usr/bin/env python3
"""무엇이 지워지는지 말하고 묻는가 — 축출 고지의 전 경로 + 토큰 결박.

사용자 지정 최상위 원칙: "손실 발생 가능성이 있는 행동은 명확하게 인지시키고 그래도 수행할지
물어야 함." 백업 링은 게임마다 `store.BACKUP_KEEP`칸을 공유한다. 포화 링에 새 백업이 추가되면
`backup_order_key` 순서의 꼬리가 밀린다.
...
이 파일이 잠그는 것:
  ...
  ⓔ 토큰이 링에 묶인다 — 확인창을 띄운 사이 링이 돌면 낡은 토큰은 거부되고
     새 목록으로 다시 묻는다. 적용·저장·복원 세 경로 전부.
  ⓕ 다게임 토큰도 축출 대상에 묶인다 — 일괄 적용·전체 초기화.
     그 둘은 이름 대신 수를 말하지만, 그 수가 정확하려면 토큰이 개수가 아니라
     대상에 묶여야 한다: 확인 시점에 "0건"이던 게임이 실행 시점에 1건을 지우면
     "수가 정확하다"는 약속 자체가 깨진다.

ⓔ·ⓕ의 계측기 설계: 링·대상만 움직이고 다른 지문 조각은 건드리지 않는 조작을 쓴다
  (ⓔ = 다른 슬롯에 저장 / ⓕ = 설정 파일 재기록·포화 링 회전). 그러지 않으면 결박이 없는
  구현도 통과해 이 절이 아무것도 못 가른다 — ⓕ는 옛 지문이 실제로 안 움직였는지까지 잰다.
...
def main_test():                                                # noqa: C901  (시나리오 나열)
    ...
    try:
        ...
        def as_items(appid, filenames):
            ...
            `dup`(같은 표기 구분 번호)은 0으로 둔다: 이 파일의 픽스처는 파일명을 전부
        ...
        #
        #    「저장돼 있나」의 판정은 기록이 아니라 본체다(`store.evacuable_names`). 기록만 남고
        #    본체가 사라진 슬롯을 묻는 갈래로 보내면, 확인창이 "지금 저장돼 있는 것"이라며
        #    이미 없는 내용을 설명한다. 이 절이 지키는 것은 그 다음이다 —
        ...
        # 940은 대피가 없는 게임이다(디스크 = dock) — 예고에 들어가면 안 된다.
        ...
        #    계측기: 링을 돌리는 조작으로 다른 슬롯 저장을 쓴다. 그러면
        ...
        def rotate_ring(appid):
            ...
            내용이 같으면 `save_profile`이 아무것도 쓰지 않아 링이 안 돈다. 그러면 낡은
              토큰이 통과하는 것이 옳은 동작이 되어 이 절이 거짓 FAIL을 낸다. 그래서 회차
              번호로 내용을 갈라 두고, 돌았는지 여기서 확인한다.
            """
            rotations[0] += 1
        ...
        # 복원 — 목록 꼬리의 행은 고르지 않는다(링을 돌리면 그 행 자체가 사라져 다른 조각이
        ...
        # ⓕ 다게임 토큰도 축출 대상에 묶인다
        #
        #   일괄 적용·전체 초기화 토큰의 링 신호를 `remove.reset_fingerprint` 하나로 두면 두
        #   갈래로 뚫린다 — 그 지문의 링 신호는 게임별 개수(`bk=%d`)뿐이고 설정 파일 sha는
        #   들어 있지 않다:
        #     ⓕ-1 확인 시점에 "축출 0건"이던 게임의 설정 파일을 게임·클라우드가 고유 내용으로
        #         바꾸면, 낡은 토큰이 통과하고 실행이 대피본을 만들며 고지에 없던 백업이 지워진다.
        #     ⓕ-2 포화 링이 한 칸 돌면 개수는 상한 그대로라 대상이 바뀌어도 안 걸린다.
        #
        #   계측기 설계: 두 절 모두 `reset_fingerprint`가 변하지 않는 조작만 쓴다. 그리고
        ...
        if first.get("code") != codes.CONFIRM_REQUIRED:
            ...
            # 게임(또는 클라우드)이 확인창을 띄운 사이 설정 파일을 고유 내용으로 다시 썼다.
        ...
        if first.get("code") != codes.CONFIRM_REQUIRED:
            ...
            fp_before = remove.reset_fingerprint(store.load_registry())
            # 링을 한 칸 돌린다 — 포화 상태라 개수는 상한 그대로이고 이름만 바뀐다.
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ⓓ 전체 초기화 — 예고한 건수가 실제 삭제 건수와 같다 (맨 마지막: 세계를 지운다)
        # ═══════════════════════════════════════════════════════════════════
        main._CONFIRM_TOKENS.clear()
