#!/usr/bin/env python3
"""대피의 중복 판정은 동작이 시작할 때 한 번이다.

### 무엇이 틀렸었나
등록 해제·전체 초기화는 두 슬롯을 차례로 백업 링에 대피시킨다. 판정이 호출마다 따로 나면,
먼저 쓴 대피본이 링을 밀어내면서 뒤 항목이 「이미 있다」고 믿던 바로 그 사본을 잘라 낸다 —
그러면 그 프로필 내용이 슬롯에도 백업에도 없다. 중복인 쪽이 뒤면 대신 예고보다 백업이 하나 더
지워진다. 이 파일은 되돌리면 반드시 FAIL이 나야 값이 있다.

### 픽스처의 함정 — 중복 판정은 태그별이다
근거 사본의 태그가 `disk`면 프로필 대피의 근거가 되지 못한다(되돌릴 곳이 다른 두 행은 서로를
대신하지 못한다). 그것을 모르고 링을 `disk` 사본으로만 채우면 승격 갈래에 한 번도 안 닿는다.
그래서 근거 사본은 `profile_dock`/`profile_internal` 태그로 놓고, 축출 꼬리에 들어오게 배치한다.

### 이 파일이 잠그는 것
  ⓐ 중복인 쪽이 앞(dock)일 때 — 셋이 같이 서야 잰 것이다:
     ① 그 프로필 내용이 백업에 남는가 ② 예고한 개수 == 실제 ③ 예고한 이름 == 실제 지워진 이름
  ⓑ 중복인 쪽이 뒤(internal)일 때 — 같은 셋.
  ⓒ 음성 대조군 — 승격이 일어나면 안 되는 세계. 근거 사본이 축출 꼬리 밖(링 중간)이면 그것은
     살아남으므로 근거가 유효하고, 아무것도 승격하지 않는다. 이 세계가 없으면 "그냥 항상 쓴다"는
     구현이 ⓐⓑ를 초록으로 통과한다. 멀쩡한 백업이 같은 이름 그대로 남는지도 여기서 잰다.
  ⓓ 계획↔쓰기 창(저장·삭제 두 갈래). 계획은 본체를 `sha1_file`로 재고 쓰기는 `read_bytes`로
     다시 읽는다. 그 사이 본체가 바뀌면 실제 키가 계획에 없다 — 거기서 무쓰기로 접으면 대피 없이
     저장이 슬롯을 덮고 삭제가 `rmtree`까지 간다. 계획은 "이것만 써라"가 아니라 "이건 중복이라도
     써라"는 허가 목록이어야 한다. 지금 코드에는 없는 구멍이라 여기서 FAIL하지 않는 것이 정상이고,
     미래의 재도입을 막는 자리다.
  ⓔ 링 상한은 그대로다 — 승격이 늘어도 `BACKUP_KEEP`칸을 넘지 않는다.
  ⓘ 비포화 링에서는 아무도 안 밀려난다 — 그때 증인은 믿어야 한다. ⓒ의 형제이고 경계를 재는
     쪽이다: ⓒ는 근거 사본을 링 한가운데 두어 "살아남는 꼬리의 경계"를 안 잰다. 여기서는 링이
     `BACKUP_KEEP`보다 한 칸 적고 근거가 꼬리 끝이라, 꼬리를 한 칸이라도 과대평가하면 곧바로
     증인을 불신한다. 그러면 정상 중복이 쓰이고 확인창이 예고하지 않은 백업이 삭제된다.
     판정을 `BACKUP_KEEP - len(plan)` 꼴로 쓰면 링이 가득 찼다고 가정하는 것이라 여기서 깨진다.
  ⓚ 링이 상한을 넘고 계획이 0건이면 아무것도 안 밀려난다 — 행동으로 잰다. `prune_backups`는
     `make_backup` 안에서 쓰기 뒤에만 돈다. 한 건도 안 쓰면 prune 자체가 안 돌아, 상한을 넘긴
     링이라도 그대로 남는다. 「어차피 잘린다」로 읽으면 예고 0인데 백업이 사라진다.
     이 세계가 ⓙ의 사각을 덮는다 — 실제 `make_backup` 호출을 지나므로 술어가 죽은 헬퍼가 되거나
     이름이 바뀌면 여기서 걸린다.
  ⓙ 「살아남는 칸」 계산이 정의와 어긋나지 않는다 — 특히 계획이 링보다 클 때.
     `entries[:len(entries) - doomed]` 꼴로 적으면 `doomed`가 링 칸수를 넘는 순간 인덱스가 음수가
     되고, 파이썬 음수 슬라이스는 0개가 아니라 앞부분을 다시 신뢰한다. `max(0, …)`가 슬라이스
     인덱스 자리에 있어야 막힌다. 이 세계는 단위 단언이다 — `len(plan) > BACKUP_KEEP` 구간에서는
     기존 링이 어차피 전부 축출되므로 행동으로는 차이가 안 난다. ⓔ(본체가 상한보다 많은 세계)로도
     못 잡는다: 거기선 모든 본체가 새 내용이라 전부 계획에 들어가 계획 밖 갈래에 안 닿는다.
  ⓖ 계획 밖 키의 자체 판정도 「축출될 사본」을 근거로 삼지 않는다. ⓓ와 같은 「계획 뒤 본체가
     바뀐다」인데 링이 포화이고 꼬리 끝 칸이
     바뀐 내용의 유일한 사본일 때 무쓰기로 접으면 뒤 슬롯의 쓰기가 그 칸을 축출해 전손이 된다.
     저장 갈래로는 재현되지 않는다(대피가 한 슬롯뿐이라 뒤따르는 쓰기가 없다).
  ⓗ 판정은 「동작 경계에서 한 번」이다 — 횟수로 잰다. 승인 1회는 `plan_backups`를 정확히 두 번
     (고지 1 + 실행 1) 부른다. `>= 1`로만 재면 계획 호출을 하나 더 넣은 구현이 그대로 통과한다.
  ⓛ 한 동작이 둘 이상 쓸 때, 먼저 쓴 대피본이 뒤 쓰기의 prune에 잘리지 않는다.
     링에 지금보다 미래 stamp를 가진 백업이 있으면 새 대피본이 `backup_order_key` 기준 꼬리로
     읽힌다 — 자기 하나만 보호하는 prune은 그것을 다음 쓰기에서 잘라 내고, 그 슬롯은 곧
     `rmtree`된다(전손). 그런데 결과 봉투는 `evacuated`로 대피시켰다고 보고한다.
     음성 대조군이 붙는다(정상 시각 링) — 거기서는 예고도 실제도 그대로여야 한다.
     삭제(등록 해제)와 「본체가 둘 이상인 슬롯 저장」을 둘 다 지난다 — 계획 객체를 만드는
       호출부가 `remove`·`engine` 둘이라, 한쪽만 되돌린 사본이 다른 쪽 세계를 그대로 통과한다.
     이 세계가 잠그는 것은 「한 동작 안의 순서」뿐이다. 링 순서를 파일명 벽시계 stamp에서
       유도한다는 전제는 그대로 남고 화면의 시각 표기·목록 정렬도 그대로다 — 여기가 초록이라고
       "시각 축을 닫았다"로 읽으면 화면도 문서도 거짓이 된다.
  ⓕ 대피는 「할 수 있을 때」 한다 — 슬롯 폴더를 열거하지 못해도 저장은 성립한다.
     `store.atomic_write`는 `os.replace`가 `try` 안이고 디렉터리 fsync는 밖이라, 그 fsync 실패를
     예외로 올리면 본체를 이미 덮은 뒤에 쓰기 실패를 말하게 된다: 화면은 "프로필 정보가 손상되어
     저장할 수 없습니다"라 하는데 기록만 옛 내용을 가리켜 그 프로필은 적용도 안 된다.
     범위가 넓어서 잠근다 — `atomic_write`는 제품 호출부가 일곱이라, 무심코 `raise`가 되살아나면
     앱이 파일을 쓰는 모든 자리가 같이 영향받는다.

### 계측 — 경로가 실제로 몇 번 돌았는지 센다
`store.make_backup`을 감싸 호출 시점의 링으로 `backup_holds`를 먼저 재고, 그 뒤 실제로 파일이
만들어졌는지 본다. 「중복인데 썼다」 = 승격이다. 0회면 이 파일은 아무것도 안 잰 것이고, 그 사실을
화면에 찍는다(0회면 FAIL로 끝난다 — 계측기 무효를 통과로 읽지 않는다).
...
#: ⓛ 전용 — 한 슬롯에 본체가 둘인 상태를 만들어 저장이 백업을 둘 쓰게 한다.
...
def instrument(store):
    ...
    def wrapped(appid, data, tag, filename, **kw):
        ...
        if out:
            ...
            if plan is not None and not in_plan:
                COUNT["plan_fallback"] += 1     # 계획 밖 키를 스스로 판정해 썼다
        else:
            COUNT["skipped"] += 1
        return out
...
def instrument_plan(store):
    ...
    「쓸 때 중복이었는가」만 세면 중복인 쪽이 뒤일 때 승격을 못 본다: 먼저 쓴 대피본이
      근거 사본을 이미 밀어내 버려서, 두 번째 쓰기 시점에는 중복이 아니다. 그것이 바로 이
      결함의 기전이므로, 승격은 판정 시점(계획을 낼 때 중복인데도 계획에 넣었는가)에서 센다.
    `plan_backups`가 없는 판본에서는 감싸지 않는다 — 그때는 아래 관측 단언(①②③)이 이미
      FAIL한다. 계측이 없다고 통과로 읽히지 않는다.
    """
...
def ring_survivors(entries, adding, keep):
    ...
    생산 코드의 식(`_doomed_tail`)을 인용하지 않는다. 그것을 오라클로 쓰면 항진식이 되어
      "코드로 코드를 재게" 된다. 대신 링의 동작 자체를 흉내 낸다: `prune_backups`는 이번
      동작의 보호 경로를 쓴 역순으로 앞세우고, 나머지를 `backup_order_key` 순서로 이은
      목록의 꼬리를 자른다. 동작 밖 순서는 파일명 벽시계 stamp·seq·이름에서 유도된다.
    쓰기가 0건이면 아무도 안 밀린다 — `prune_backups`는 `make_backup` 안에서 쓰기 뒤에만
      돌기 때문이다. 링이 `keep`을 넘겨 있어도 그렇다.
    전제: 백업 stamp가 시각순으로 단조 증가한다. 동작 밖 순서를 그 stamp에서 읽는 이 오라클의
      뼈대는 그 위에서만 성립한다 — 미래·역행 stamp가 섞이면 순서가 뒤집혀 전제가 깨진다.
    """
    if adding <= 0:
        return list(entries)
...
def main_test():                                                # noqa: C901  (세계 나열)
    ...
    try:
        ...
        for tag, (appid, dup_profile, dup_data, live_profile, live_data) in {
            ...
            build(appid)
            witness(appid, dup_profile, dup_data)         # 꼬리 끝 = 근거 사본
        ...
        # ── ⓘ 비포화 링 + 근거가 꼬리 끝 → 무쓰기 · 축출 0 ────────────────────
        #    링이 `BACKUP_KEEP`보다 적으면 이 동작이 써도 아무도 안 밀려난다. 그러면 꼬리 끝
        #    사본도 살아남으므로 믿어야 한다. 여기서 불신하면 정상 중복이 쓰이고, 확인창은
        #    축출 0건이라 아무 이름도 대지 않았는데 백업이 하나 사라진다.
        appid = "9109"
        build(appid)
        witness(appid, "dock", DOCK)                  # 꼬리 끝 = dock 내용의 근거 사본
        ...
        # ── ⓙ 「살아남는 칸」 계산 — 격자 전수 ────────────────────────────────
        #    정의: 이 쓰기를 안 했을 때 살아남는 칸 = 계획이 0건이면 링 전부,
        #    아니면 `max(0, 링 - max(0, 링 + 계획 - KEEP))`.
        ...
        if trusted_fn is None:
            ...
            for ring in (0, 1, 9, store.BACKUP_KEEP, store.BACKUP_KEEP + 2):
                ...
                for size in (0, 1, 2, store.BACKUP_KEEP - 1, store.BACKUP_KEEP,
                    ...
                    checked += 1
                    if got != want:          # 개수가 아니라 항목을 본다(같은 수의 꼬리를 줘도 걸린다)
            ...
            # 오라클이 항진식이 아님을 음성 대조군으로 보인다: 틀린 답을 주면 걸려야 한다.
        ...
        # ── ⓖ 계획 밖 키의 판정도 축출될 사본을 근거로 삼지 않는다 ──────────────
        #    ⓓ와 같은 「계획 뒤 본체가 바뀐다」이지만 링이 포화이고 꼬리 끝 칸이 바뀐 내용의
        #    유일한 사본이다. 계획 밖 판정이 「호출 시점의 링」을 보면 그 칸을 근거로 삼아
        #    무쓰기가 되고, 뒤 슬롯의 계획된 쓰기가 바로 그 칸을 축출한다 — 내용이 슬롯에도
        #    링에도 없다(전손).
        #    저장 갈래로는 재현되지 않는다: 저장은 대피가 한 슬롯뿐이라 뒤따르는 쓰기가 없어
        #      근거 사본이 축출되지 않는다. 두 슬롯을 차례로 옮기는 삭제라야 도달한다.
        appid = "9108"
        build(appid)
        witness(appid, "dock", MOVED)                 # 꼬리 끝 = 바뀐 뒤 내용의 유일한 사본
        for i in range(store.BACKUP_KEEP - 1):
            filler(appid, 200 + i)
        ...
        # ── ⓕ 대피는 할 수 있을 때 한다 — 열거 못 해도 저장이 성립한다 ──────────
        #    `0300` = 쓰기·통과는 되고 열거만 막힌다. 그래서 `evacuable_names`가
        #    조용히 빈 목록이 되어 대피가 0건이 되고, 그 다음 디렉터리 fsync가 실패한다.
        #    잃는 것은 「이름의 지속」이지 내용의 온전성이 아니다 — 그래서 이 세계는
        ...
        # ── ⓛ-3·4 본체가 둘 이상인 슬롯 저장도 같은 자리다 (engine 쪽 호출부) ──
