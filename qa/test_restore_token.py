#!/usr/bin/env python3
"""복원 토큰이 목적지와 백업 파일에 결박되는가.

### 무엇이 문제였나
토큰의 슬롯 2가 `"restore_backup"` 한 마디뿐이던 시절, 어느 백업 행인지도 어디로 되돌리는지도
안 묶였다. 그래서 두 슬롯의 내용이 같고 두 프로필 대피본의 내용도 같으면, dock 확인창에서 받은
토큰으로 internal 슬롯을 덮어쓸 수 있었다 — 사용자가 확인한 곳과 다른 곳에 쓰기가 일어난다.
지금 슬롯 2는 `_restore_scope` = `restore_backup:{target}:{backup_id}`다.

### 이 테스트가 세우는 세계 (그 재현을 그대로 만든다)
    dock 슬롯 = internal 슬롯 = `C`                    → 되돌릴 곳의 지문이 바이트 동일
    profile_dock 백업 = profile_internal 백업 = `R`     → 백업 쪽 지문도 바이트 동일
즉 토큰의 네 칸 중 세 칸이 같고, 갈리는 것은 슬롯 2의 합성 문자열뿐이다.
결박이 없으면 토큰이 그대로 넘어간다 — 이 테스트가 재는 것이 정확히 그 한 칸이다.

잠그는 것:
  ⓐ dock 확인 토큰으로 internal 백업 호출 → 거부
  ⓑ 같은 조합에서는 통과한다 (= "아무 토큰이나 거부"가 아님을 증명하는 음성 대조군)
  ⓒ 같은 목적지·같은 지문의 다른 backup_id로 재사용 → 거부
  ⓓ 위조·만료 토큰 → 거부
  ⓔ ⓐ~ⓓ의 거부 구간에서 tmp 아래 전 파일의 sha1이 한 바이트도 안 바뀐다

못 재는 것(정직):
  · TTL 만료의 실시간 경과는 재지 않는다 — `_CONFIRM_TTL`을 0으로 낮춰 대신 잰다.
  · 토큰을 사람이 실제로 봤는지는 신뢰 경계 밖이다.
...
APPID = "555"
#: 두 슬롯이 공유할 내용(C)과 두 대피본이 공유할 내용(R).
#: 길이를 맞춘다 — `engine.check_sanity`가 크기 25~400%를 본다.
C = b"quality=comm\nshadows=same\nsource=BOTH-SLOTS-C\n"
R = b"quality=roll\nshadows=back\nsource=BOTH-BACKUPS\n"
...
def world_sha(tmp):
    """tmp 아래 전 파일의 (상대경로 → sha1). 거부가 정말 무쓰기인지를 효과로 잰다."""
    import hashlib
    out = {}
...
def build_world(tmp, engine, store):
    """재현 세계를 실제 저장 경로로 만든다. 예외는 아래 한 파일뿐이다.

    끝난 상태:
        dock 슬롯 = C · internal 슬롯 = C
        백업 = profile_dock(R) · profile_dock(C) · profile_internal(R) · profile_dock(R)
    `profile_dock` R 백업이 두 건인 것이 ⓒ(다른 backup_id 재사용)를 재는 재료다 —
    둘은 내용도 목적지도 같고 이름만 다르다.

    그 두 번째 건만은 손으로 놓는다. 지금 `store.make_backup`은 같은 태그에 같은 내용이 링에
      있으면 아무것도 쓰지 않으므로 정상 경로로는 그런 쌍이 생기지 않는다. 그런데 ⓒ가 재는
      계약은 여전히 살아 있다 — 그 규칙은 앞으로 만들 백업에만 걸리고, 링을 소급해서 정리하는
      코드는 없다(`store.prune_backups`는 칸 수로만 자른다). 그래서 픽스처만 그 시절의 링을
      재현한다. 놓는 방식은 정상 명명 규칙(`<stamp>-<tag>-<filename>`)을 그대로 따르는 파일
      하나이고, 그 밖의 모든 상태는 실제 저장 경로가 만든다.
    """
    ...
    cfg.write_bytes(C)
    engine.save_profile(reg, APPID, "dock")        # 대피 안 씀(profile_dock(R)이 이미 있다) · dock = C
    cfg.write_bytes(R)
    ...
    # 위 독스트링이 예고한 한 건 — `profile_dock(R)`의 두 번째 건을 재현한다.
    #   stamp를 아주 예전으로 두어 목록 맨 끝에 놓는다. `store.atomic_write`는 prune을 부르지
    #   않고, 이 뒤로도 링이 `store.BACKUP_KEEP` 아래라 나중 prune에도 안 잘린다.
...
def main_test():                                                # noqa: C901  (시나리오 나열)
    ...
    try:
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ⓒ 같은 목적지·같은 지문의 다른 backup_id로 재사용
        # ═══════════════════════════════════════════════════════════════════
        main._CONFIRM_TOKENS.clear()
        env = rpc(main, "restore_backup", APPID, dock_r[0])
        tok_dock = params_of(env).get("confirm_token")
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ⓔ 여기까지 거부만 있었다 → tmp 아래가 한 바이트도 안 바뀌었어야 한다
        # ═══════════════════════════════════════════════════════════════════
        after = world_sha(tmp)
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ⓑ 음성 대조군 — 같은 조합에서는 통과한다 (아무 토큰이나 거부하는 것이 아니다)
