...
### 이 파일이 잠그는 것 일곱
  ⓐ 같은 태그에 같은 내용이 있으면 **아무것도 쓰지 않는다** — 그리고 안 쓰므로 **안 지운다**.
    ...
    **중복이면 거짓·새 내용이면 참**인가. 백엔드가 내는 값만 잰다 — 화면이 그 값으로 줄을 가리는가는
    ...
    ★★ 그 판정을 **포화 링에서만 재면 아무것도 못 잰다**(QA R2 N-2): 거기서는 「쌓이는가」와
    「잘리는가」가 언제나 같이 참·같이 거짓이라 둘을 바꿔 놓아도 검사가 안 움직인다. 그래서
    **비포화 링 세계**(ⓐ-3)를 같이 세운다 — 거기서만 `adding≠0`이면서 `evicted==[]`가 된다.
...
def main_test():                                                # noqa: C901  (시나리오 나열)
    ...
    try:
        ...
        #     ★★ 이 세계가 없으면 위 둘은 `adding`과 `evicted`를 **구별하지 못한다**(QA R2 N-2):
        #       포화 링에서는 `adding≠0 ⟺ evicted≠0`이 항상 참이라, `evacuates`를 축출 목록으로
        #       판정하는 변이(`bool(ring["evicted"])`)가 초록으로 지나간다. 그 회귀가 들어오면
        #       **평범한 게임**(백업 10칸 미만)에서 적용이 백업을 한 칸 쓰면서 **그 사실을 침묵한다** —
        #       이 프로젝트가 스스로 적어 둔 *"침묵한 삭제가 과하게 예고한 삭제보다 나쁘다"*
        #       (`store.pending_backups`)의 나쁜 쪽이고, D-3이 고친 거짓말의 **정확한 반대 방향**이다.
