#!/usr/bin/env python3
"""백업 링은 태그마다 같은 내용을 하나만 담는다.

내용을 안 보고 매번 새 파일을 만들면, dock ↔ internal 두 내용 사이를 왕복하는 디스크 설정이
같은 내용으로 링을 채운다 — 링을 마모시키는 것은 실패가 아니라 정상 동작이다. 그래서 중복
판정의 문을 `store.backup_holds` 하나로 모았고, 이 파일은 그 문의 양쪽을 잰다.

이 파일이 잠그는 것 일곱
  ⓐ 같은 태그에 같은 내용이 있으면 아무것도 쓰지 않는다 — 그리고 안 쓰므로 안 지운다.
    (음성 대조군: 내용이 새것이면 정확히 1건 쌓이고 1건 잘린다. 둘이 같이 서야 잰 것이 「중복」이다.)
    적용 확인창의 `evacuates`도 여기서 잠근다: "백업 한 칸을 씁니다"가 참인 조건이
    중복이면 거짓·새 내용이면 참인가. 백엔드가 내는 값만 잰다 — 화면이 그 값으로 줄을 가리는가는
    Gamepad UI에만 있어 실기 몫이다.
    그 판정을 포화 링에서만 재면 아무것도 못 잰다: 거기서는 「쌓이는가」와 「잘리는가」가 언제나
    같이 참·같이 거짓이라 둘을 바꿔 놓아도 검사가 안 움직인다. 그래서 비포화 링 세계(ⓐ-3)를
    같이 세운다 — 거기서만 `adding≠0`이면서 `evicted==[]`가 된다.
  ⓑ 태그가 다르면 같은 내용도 담긴다. 되돌릴 곳이 다른 두 행은 서로를 대신하지 못한다 —
    전역으로 걸렀다면 한쪽 태그의 행이 통째로 사라지고 화면은 그것을 "이쪽은 백업이 없다"로 읽는다.
  ...
  ⓓ 고지한 개수 == 실제로 지워진 개수. 본체가 둘인데 내용이 같으면 실제로 쌓이는 것은
    한 건이므로, 확인창도 한 건만 예고해야 한다(둘을 예고하면 일어나지 않을 삭제를 말한다).
  ...
  ⓕ 링크된 백업도 판정에 끼지 않는다. 엔진 G15가 링크 복원을 거부하므로 그것으로는 못
    ...
    (이 픽스처는 정상 API로 못 만든다. 그래서 직접 놓는다.)
  ⓖ 「형식 불명」의 정의는 `parse_backup_id` 하나다. 순서 키가 자기 정규식을 따로 쓰면
    화면은 형식 불명이라는데 정렬은 정상 백업인 이름이 생기고, prune이 자르는 파일과
    화면이 대는 이름이 갈린다.
...
def main_test():                                                # noqa: C901  (시나리오 나열)
    ...
    try:
        ...
        if ask is not None:
            ...
            #   조건이다. 중복이면 `make_backup`이 아무것도 안 쓰므로 칸을 안 쓴다 — 그런데
            #   이 필드가 없거나 참으로 고정되면 가장 자주 뜨는 확인창이 사용자에게 정반대를
            #   말한다.
            #   여기서 재는 것은 백엔드가 내는 값이다. 그 값을 화면이 실제로 가리는가는
            #     Gamepad UI에만 있어 검사로 못 잰다(실기 몫).
        ...
        if ask is not None:
            # `evacuates`의 음성 대조군: 새 내용이면 참이어야 한다. 이것이 없으면 위
        ...
        # ⓐ-3 비포화 링: 칸이 남으면 대피는 일어나지만(`adding`) 아무것도 안 잘린다(`evicted`).
        #     이 세계가 없으면 위 둘은 `adding`과 `evicted`를 구별하지 못한다: 포화 링에서는
        #       `adding≠0 ⟺ evicted≠0`이 항상 참이라, `evacuates`를 축출 목록으로 판정하는
        #       변이(`bool(ring["evicted"])`)가 초록으로 지나간다. 그 회귀가 들어오면 백업이
        #       상한에 못 미치는 게임에서 적용이 백업을 한 칸 쓰면서 그 사실을 침묵한다 —
        #       예고하지 않은 손실이라 과하게 예고한 쪽보다 나쁜 방향이다.
        cfg866 = build("866", "NotSaturated")                     # 첫 저장 둘 = 대피 없음 → 링 0칸
        if len(names("866")) >= store.BACKUP_KEEP:
            P("ⓐ-3 계측기 무효 — 비포화 세계를 만들지 못했다(링 %d칸)" % len(names("866")))
        cfg866.write_bytes(E1)                                    # 어느 슬롯과도 다른 새 내용
        ask, (added, gone) = apply_once("866", "dock", "ⓐ-3")
        if ask is not None:
            if ask.get("evacuates") is not True:
                P("★ⓐ-3 링에 칸이 남아 대피본이 실제로 쌓이는데 `evacuates`=%r다 — 확인창이 "
                  "**백업 한 칸을 쓰면서 침묵한다**(축출 목록으로 판정하면 이 값이 된다)"
                  % ask.get("evacuates"))
            if ask.get("evicted"):
                P("★ⓐ-3 칸이 남는데 축출을 예고했다 — %s" % ask.get("evicted"))
        if len(added) != 1 or gone:
            P("★ⓐ-3 계측기 무효 — 비포화 링의 적용이 %d건 추가·%d건 축출했다(1·0이어야 한다)"
              % (len(added), len(gone)))
        ...
        # 본체 둘은 앱이 만드는 상태가 아니다: 같은 appid 재등록은 등록 route가
        #   `confirm.already_registered`로 거부하고, 슬롯 복원은 그 슬롯이 쓰던 이름을 유지한다.
        #   밖에서 들어온 파일이나 중단된 삭제의 잔재로만 생기므로 여기서는 직접 놓아 만든다 —
        ...
        #    순서 키가 tag를 스스로 판정하면, 우리가 만들지 않는 tag를 단 파일이 화면에서는
        #    형식 불명인데 정렬에서는 정상 백업이 된다 — prune이 자르는 순서와 화면이 대는
        #    이름이 갈린다.
        # ═══════════════════════════════════════════════════════════════════
        ...
        # 정의가 갈리면 무엇이 잘리는가까지 본다: stamp가 아주 미래인 형식 불명 파일은
        #   순서 키가 그것을 정형으로 읽는 순간 맨 앞이 되어 살아남고, 대신 앱이 만든 진짜
        #   백업 하나가 잘려 나간다.
        build("865", "OrderKey")
        saturate("865")
        fillers = names("865")
