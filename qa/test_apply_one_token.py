#!/usr/bin/env python3
"""**개별 적용의 2단계 계약과 토큰** — 설계 §5-E-2 · §5-E-4.

13판에서 `apply_profile`이 2단계가 됐다: 두 슬롯 **모두와 다른** 게임 설정 파일을 덮어쓰는 것은
*"이 파일에만 있는 내용이 **사라질 수 있는**"* 갈래라 확인을 요구한다.
⚠️ **「유일한」이 아니다**(2026-08-22 정정): 여기 온다고 반드시 잃는 것은 아니다 — 슬롯 본체가
깨진 상태도 이 갈래로 오고, 그때는 승인해도 엔진이 `PROFILE_CORRUPT`로 거부해 아무것도 안 바뀐다
(설계 §15-D E10). 계약은 *"잃을 수 있으면 묻는다"*이지 *"물으면 잃는다"*가 아니다.

잠그는 것:
  ① **무토큰 1차 호출은 파일을 1바이트도 안 바꾼다**(파일계 전수 sha1로 잰다)
  ② 위조·재사용·만료 토큰 거부
  ③ **scope 혼동** — 저장·복원·일괄 적용 토큰으로 개별 적용이 통과하지 않는다
  ④ ★ **방향 바인딩** — dock 확인 토큰이 internal 적용에 소비되지 않는다
     (음성 대조군: **원래 방향에서는 통과한다**)
  ⑤ **TOCTOU** — 확인창 사이에 게임이 설정을 다시 쓰면 낡은 토큰은 거부된다

### ④를 재려면 세계가 특별해야 한다 (그러지 않으면 잰 것이 지문이다)
두 슬롯의 내용을 **같게** 만든다. 그러면 두 방향의 지문 `(meta sha1, disk sha1)`이 바이트 동일해서
**토큰에서 갈리는 것은 슬롯 2의 합성 문자열(`apply_profile:{profile}`)뿐**이다.
내용이 다르면 지문이 달라 거부되는데, 그건 방향 결박을 잰 것이 아니다.
